        ...
        **** 위의 댓글처리를 하고 나서 느낀점
        ...
        ck editor 적용중
            ...
            이미지 업로드 진행중
                ...
                1. 이미지수정이 없을경우
                2. 이미지를 전부 수정하는 경우
                    수정하는 이미지는 ckeditor 통해 로컬에 쓰고 db 에 저장됨
                    수정되는 이미지는 ckeditor 통해 뷰에서 지워짐. (로컬,db잔류)
                    update 누르면 수정하는 이미지를 boardid 부여됨
                    read 에서 보이는건 수정된 이미지 뿐.
                    원래 이미지를 db 와 로컬에서 삭제해야함
                    -> upload 누른 순간 boardId = 0 으로 set
